...
- Name: api\validateAccount.spec.ts >> API - Validate Account >> TC-API-04 - new account type is SAVINGS @regression @api
- Location: tests\api\validateAccount.spec.ts:32:9
...
```ts
  ...
  6  | 
  7  |     test('TC-API-03 - new account exists in API @regression @api', async ({ request }) => {
  ...
  30 |     });
  31 | 
  32 |     test('TC-API-04 - new account type is SAVINGS @regression @api', async ({ request }) => {
...
> 38 |         expect(new_account.status()).toBe(200);
  ...
  44 |     });
  45 | 
  46 |     test('TC-API-05 - invalid customerId returns error @regression @api', async ({ request }) => {
